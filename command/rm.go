...
Examples:
	...
	12. Delete a directory-marker object (a zero-byte object whose key ends with "/")
...
func NewDeleteCommand() *cli.Command {
	cmd := &cli.Command{
		...
		Action: func(c *cli.Context) (err error) {
			...
			// Delete.Run records one stat entry per object it removes, not
			// one per command, so that a single "rm a b c" generated by
			// "sync --delete" is reported as three removals. Only failures
			// that happen before Run starts are recorded here.
			started := false
			defer func() {
				if !started {
					stat.Collect(op, &err)()
...
func (d Delete) Run(ctx context.Context) error {
	...
	// Every object removed, or that failed to be removed, is one stat entry.
