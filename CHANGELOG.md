...
#### Bugfixes
- `sync --delete` removes nothing when the source matched no object. `sync --delete s3://bucket/typo/* dir/` used to report `no object found` and then empty `dir/`; a missing prefix and an empty one look the same on S3, so the sync now refuses (exit 1) and says to use `rm` to empty a destination. Objects left out by `--exclude` count as matched. (design decision DD-8)
- `sync` keeps Glacier source objects in the comparison instead of dropping them. Their destination copies no longer look stale: `--delete --ignore-glacier-warnings` used to remove the only readable copy, exit 0. A Glacier object is reported only when a copy would be due, so a sync whose destination is already current exits 0 where it used to exit 1. `--force-glacier-transfer` is unchanged. (DD-9)
- `sync` from S3 to a local directory that holds a dangling symlink no longer stops; the link counts as absent and the file is copied over it. (DD-7, part of #44)
- The commands `sync` generates now quote any key that is not plainly safe, so a key containing a vertical tab or other unusual whitespace is no longer split by `run` (found by a new property-based test).
...
- `cp`/`sync`/`rm`/`du` from a local directory: an entry that cannot be read (a dangling symlink, a directory without read permission) no longer ends the walk of its top-level directory. It used to be reported once and every entry after it in that directory was silently left out, which is how a backup could miss whole directories and still exit 0. The entry is now reported with its path and the walk goes on. For `sync`, an unreadable entry that `--exclude`/`--include` leaves out no longer stops the sync; one that is not excluded still does. (upstream [#720](https://github.com/peak/s5cmd/issues/720))
- `sync` (and every listing) used to compile a regular expression for each object's URL, about 4 KB apiece, and `sync --delete` kept every destination-only URL for one giant `rm` command that then started a goroutine per object; a sync of a few hundred thousand objects was killed for running out of memory. Plain keys no longer compile a regexp, and `--delete` writes `rm` commands in batches of 1000 objects, so memory no longer grows with the number of objects: a `sync --delete` of 200k destination-only objects peaks at 195 MiB instead of 3 GiB. (upstream [#745](https://github.com/peak/s5cmd/issues/745))
- `sync` from a local directory: a dangling symlink (or any file entry that exists but cannot be stat'ed) is reported with its path, skipped, and the rest of the directory is still synced; the exit code is 1. It used to stop the sync with an error that did not say which entry. An unreadable directory, or a symlink to one, still stops the sync: what is under it is unknown. With `--delete`, a skipped source entry means nothing is deleted from the destination, and the sync says so - the same rule rsync applies to I/O errors on the sending side - so an old copy of the entry, or a subtree behind a broken mount, is never removed because it could not be read. (upstream [#749](https://github.com/peak/s5cmd/issues/749), [#800](https://github.com/peak/s5cmd/issues/800))
